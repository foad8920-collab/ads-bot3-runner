...
const BOT_DB_NAME = `bot${ACCOUNT_NUM}`; // 🟢 استخراج اسم البوت (bot3) لمطابقة جداول اللوحة المركزية
const BOT_GROUP_FIELD = `bot${ACCOUNT_NUM}_group`;
const BOT_STATUS_FIELD = `bot${ACCOUNT_NUM}_status`;
const BOT_AI_FIELD = `ai_final_text${ACCOUNT_NUM}`;
...
// 🔄 تصفير الحقول المعلقة الخاصة بالبوت الثالث
async function resetStuckPosts() {
    await logToDashboard(`🔄 [${ACCOUNT_NAME}] جاري فحص وتصفير حقول البوت الثالث المتبقية (${BOT_GROUP_FIELD})...`, 'info');
    const updateObj = {};
...
async function publishToGroup(page, group, post, imagePath) {
    ...
    if (!postText || postText.trim() === '') {
        await logToDashboard(`🧠 [المرحلة 5] [AI] صياغة نص جديد بالذكاء الاصطناعي لـ ${ACCOUNT_NAME} خصيصاً لمجموعة: ${group.name}...`, 'info');
...
async function processOnePost(post) {
    ...
    try {
        while (true) {
            ...
            try {
                // 🚀 تشغيل النشر بالمراحل المستقلة المتطابقة
